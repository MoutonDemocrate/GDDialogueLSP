...
import { isValidTag } from './bbcode';

import { TextDocument } from 'vscode-languageserver-textdocument';
import { DialogueDocumentManager } from './documentManager';
import { builtinTags, dialogueManagerTags, effectTags, findTag, tagToMarkdown } from './bbcode';
...
const dialogueManager = new DialogueDocumentManager();

let hasConfigurationCapability: boolean = false;
let hasWorkspaceFolderCapability: boolean = false;

// Validate BBCode tags in text and return diagnostics
function validateBBCode(text: string): Diagnostic[] {
...
function getCurrentLine(document: TextDocument, position: Position): { text: string; indent: string } {
...
connection.onCompletion(
  (params: TextDocumentPositionParams): CompletionItem[] => {
    ...
    const isKeywordStart = keywords.some(keyword =>
      textBeforeCursor.trimStart().toLowerCase().startsWith(keyword.toLowerCase())
    );
    ...
    if (bbCodeMatch) {
      ...
      return [...builtinTags, ...dialogueManagerTags, ...effectTags]
        ...
        .map(tag => {
          ...
          const paramSnippets = tag.parameters?.map((param, i) => 
            `${param.name}=\${${i + 1}:${param.name}}${i < (tag.parameters?.length || 0) - 1 ? ' ' : ''}`
          ).join('') || '';
